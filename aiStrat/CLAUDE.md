...
- **Hooks:** PreToolUse, PostToolUse, and other lifecycle hooks implement the enforcement spectrum (Deterministic Enforcement). Hooks are the deterministic enforcement layer — they fire every time regardless of context pressure. *(See `hooks/README.md` for hook specifications; create `.claude/hooks/` when implementing.)*
- **Subagents:** The Agent tool enables parallel work with coordination (Swarm Patterns).
