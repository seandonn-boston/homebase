...
import { type TrustRecord, TrustStage, type TrustStore } from "./trust-model";
...
const SEVERITY_IMPACT: Record<
	string,
	{ successDelta: number; failureDelta: number }
> = {
...
export class TrustScoringEngine {
	...
	handleFailure(
		...
		if (!record) {
			return {
				demoted: false,
				newStage: TrustStage.ManualOversight,
		...
		if (severity === "critical") {
			...
			if (from > this.config.criticalFailureDemotionStage) {
				this.store.demoteAgent(
					agentId,
					category,
					`critical failure in ${category}`,
				);
				return {
					demoted: true,
		...
		if (severity === "high" && record.consecutiveFailures >= 2) {
			if (record.stage > TrustStage.ManualOversight) {
				...
				this.store.demoteAgent(
					agentId,
					category,
					`repeated high-severity failures in ${category}`,
				);
				return {
					demoted: true,
					newStage: record.stage,
		...
		if (
			...
			this.store.demoteAgent(
				agentId,
				category,
				`3+ consecutive failures in ${category}`,
			);
			return {
				demoted: true,
				newStage: record.stage,
	...
	applyDecay(): { decayed: TrustRecord[] } {
		...
		for (const record of allRecords) {
			if (
				now - record.lastUpdated > decayThreshold &&
				record.stage > TrustStage.ManualOversight
			) {
				const from = record.stage;
