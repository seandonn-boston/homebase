...
import { type TrustRecord, TrustStage, TrustStore } from "./trust-model";
import type { TrustScoringEngine } from "./trust-scoring";
...
export class TrustPersistence {
	...
	applyDecay(maxInactiveDays: number): TrustRecord[] {
		...
		for (const record of this.store.getAllRecords()) {
			if (
				now - record.lastUpdated > threshold &&
...
export class TrustDashboard {
	...
	getFleetView(): FleetTrustView {
		...
		const autonomyMatrix = Array.from(agentMap.entries()).map(
			([agentId, categories]) => ({
				agentId,
				categories,
			}),
		...
		return {
			...
			stageDistribution: {
				[TrustStage.ManualOversight]:
					summary.byStage[TrustStage.ManualOversight],
				[TrustStage.AssistedAutomation]:
					summary.byStage[TrustStage.AssistedAutomation],
				[TrustStage.PartialAutonomy]:
					summary.byStage[TrustStage.PartialAutonomy],
	...
	getPromotionCandidates(): {
		...
		for (const record of this.store.getAllRecords()) {
			const eligibility = this.scoring.checkPromotionEligibility(
				record.agentId,
				record.category,
			);
	...
	getDecayWarnings(warningDays: number): TrustRecord[] {
		...
		return this.store
			...
			.filter(
				(r) =>
					now - r.lastUpdated > threshold &&
					r.stage > TrustStage.ManualOversight,
			);
	}
...
export class TrustOverride {
	...
	override(
		...
		if (!record)
			throw new Error(
				`No trust record for agent=${agentId} category=${category}`,
			);
...
export class TrustAwareRouter {
	...
	filterByTrust(
		...
		return agentIds.filter((id) =>
			this.meetsMinimumTrust(id, category, minimumStage),
		);
	}
...
export class TrustReporter {
	...
	generateReport(): TrustReport {
		...
		const catMap = new Map<
			string,
			{ scoreSum: number; stageSum: number; count: number }
		>();
		for (const r of allRecords) {
...
export class TrustApprovalApi {
	private scoring: TrustScoringEngine;
	private store: TrustStore;
	private deferred: Map<string, { reason: string; timestamp: number }> =
		new Map();
			...
			if (eligibility.eligible) {
				const daysSinceLastFailure =
					record.consecutiveFailures === 0
						? Math.floor(
								(Date.now() - record.lastUpdated) / (24 * 60 * 60 * 1000),
