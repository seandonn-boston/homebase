...
describe("TrustScoringEngine", () => {
  ...
  beforeEach(() => {
    store = new TrustStore({ minConsecutiveSuccesses: 5, minDecisionsForPromotion: 5 });
  ...
  describe("recordOutcome (AU-02)", () => {
    it("increments score on success with severity weighting", () => {
      const record = engine.recordOutcome("agent-1", "code", "success", "medium");
    ...
    it("decrements score on failure", () => {
      ...
      for (let i = 0; i < 5; i++) engine.recordOutcome("agent-1", "code", "success", "low");
    ...
    it("critical failure has larger impact than low", () => {
      ...
      assert.ok(scoreAfterB < scoreAfterA, "critical should have larger negative impact");
    });
  ...
  describe("canActAutonomously (AU-03)", () => {
    ...
    it("returns false at AssistedAutomation stage", () => {
      store.initializeAgent("agent-aa", ["code"], TrustStage.AssistedAutomation);
    ...
    it("returns true for medium risk at PartialAutonomy", () => {
      store.initializeAgent("agent-pa", ["code"], TrustStage.PartialAutonomy);
      assert.equal(engine.canActAutonomously("agent-pa", "code", "medium"), true);
    });

    it("returns false for high risk at PartialAutonomy", () => {
      store.initializeAgent("agent-pa", ["code"], TrustStage.PartialAutonomy);
      assert.equal(engine.canActAutonomously("agent-pa", "code", "high"), false);
    });
  ...
  describe("handleFailure / demotion (AU-04)", () => {
    ...
    it("demotes on 3+ consecutive low/medium failures", () => {
      ...
      assert.ok(record.stage < TrustStage.PartialAutonomy, "Should have been demoted from PartialAutonomy");
    });
  });

  describe("handleModelChange (AU-04)", () => {
    it("reverts all categories to stage 2", () => {
      store.initializeAgent("agent-mc", ["code", "security"], TrustStage.FullAutonomy);
    ...
    it("does not demote if already at stage 2 or below", () => {
      ...
      assert.equal(store.getRecord("agent-low", "code")!.stage, TrustStage.ManualOversight);
    });

    it("resets consecutive successes on model change", () => {
      store.initializeAgent("agent-mc2", ["code"], TrustStage.FullAutonomy);
      for (let i = 0; i < 5; i++) store.recordDecision("agent-mc2", "code", "success");
      engine.handleModelChange("agent-mc2");
      assert.equal(store.getRecord("agent-mc2", "code")!.consecutiveSuccesses, 0);
    });
  });

  describe("handleSecurityIncident (AU-04)", () => {
    it("reverts security categories to stage 1", () => {
      store.initializeAgent("agent-sec", ["security", "code"], TrustStage.FullAutonomy);
      engine.handleSecurityIncident("agent-sec");
      assert.equal(store.getRecord("agent-sec", "security")!.stage, TrustStage.ManualOversight);
    });

    it("leaves non-security categories untouched", () => {
      store.initializeAgent("agent-sec2", ["security", "code"], TrustStage.FullAutonomy);
      engine.handleSecurityIncident("agent-sec2");
      assert.equal(store.getRecord("agent-sec2", "code")!.stage, TrustStage.FullAutonomy);
    });
  });

  describe("checkPromotionEligibility (AU-05)", () => {
    it("returns eligible when all criteria met", () => {
      store.initializeAgent("agent-promo", ["code"], TrustStage.AssistedAutomation);
      for (let i = 0; i < 6; i++) store.recordDecision("agent-promo", "code", "success");
  ...
  describe("requestPromotion (AU-05)", () => {
    it("promotes when eligible", () => {
      ...
      for (let i = 0; i < 6; i++) store.recordDecision("agent-p", "code", "success");
  ...
  describe("applyDecay", () => {
    it("decays inactive records by one stage", () => {
      ...
      store.initializeAgent("agent-decay", ["code"], TrustStage.PartialAutonomy);
      ...
      assert.equal(store.getRecord("agent-decay", "code")!.stage, TrustStage.AssistedAutomation);
    });
    ...
    it("does not decay ManualOversight records", () => {
      ...
      const result = decayEngine.applyDecay();
      const decayedIds = result.decayed.map((r) => `${r.agentId}::${r.category}`);
