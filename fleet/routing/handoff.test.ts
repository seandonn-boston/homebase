...
describe("HandoffValidator", () => {
	...
	describe("validate", () => {
		...
		it("detects missing required outputs", () => {
			...
			assert.ok(
				result.violations.some((v) =>
					v.includes('Missing required output: "tests"'),
				),
			);
		});
		...
		it("detects missing required context", () => {
			...
			assert.equal(result.valid, false);
			assert.ok(
				result.violations.some((v) => v.includes("acceptanceCriteria")),
			);
		});
...
describe("HandoffProtocol", () => {
	...
	describe("createHandoff", () => {
		it("creates a payload with unique id", () => {
			const h1 = protocol.createHandoff(
				"implementer",
				"reviewer",
				"task-1",
				{ taskId: "task-1" },
			...
			const h2 = protocol.createHandoff(
				"implementer",
				"reviewer",
				"task-1",
				{ taskId: "task-1" },
	...
	describe("getHistory", () => {
		it("filters by taskId when provided", () => {
			...
			const h2 = protocol.createHandoff(
				...
				"task-2",
				{ taskId: "task-2" },
				{ reviewResult: "pass", approvedFiles: [] },
			);
