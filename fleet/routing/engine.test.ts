...
const TEST_FILE_OWNERSHIP: FileOwnershipRule[] = [
  ...
  { pattern: "src/components/ui/**", owner: "design-systems-agent", priority: 20 },
...
const TEST_CAPABILITIES: AgentCapability[] = [
  ...
  },
  {
    ...
    modelTier: "high",
    capabilities: ["vulnerability-analysis", "penetration-testing", "compliance", "owasp"],
  },
  {
    agentId: "qa-agent",
    role: "QA Agent",
...
describe("RoutingEngine", () => {
  ...
  describe("routeByFileOwnership", () => {
    ...
    it("resolves overlapping patterns by priority (higher wins)", () => {
      ...
      const result = engine.routeByFileOwnership(["src/components/ui/Button.tsx"]);
      assert.ok(result);
  ...
  describe("routeByCapability", () => {
    ...
    it("returns null when no capabilities match", () => {
      const result = engine.routeByCapability(["quantum-computing", "teleportation"]);
      assert.equal(result, null);
    });
  ...
  describe("validateRoute", () => {
    ...
    it("flags missing acceptance criteria", () => {
      ...
      assert.ok(violations.some((v) => v.includes("Missing acceptance criteria")));
    });
  ...
  describe("DEFAULT_ROUTING_RULES", () => {
    ...
    it("all rules have required fields", () => {
      for (const rule of DEFAULT_ROUTING_RULES) {
        ...
        assert.ok(rule.primaryAgent, `Missing primaryAgent for ${rule.taskType}`);
        assert.ok(rule.escalationTarget, `Missing escalationTarget for ${rule.taskType}`);
        assert.ok(Array.isArray(rule.constraints), `constraints not array for ${rule.taskType}`);
      }
    });
