...
describe("FleetScaler", () => {
	...
	describe("evaluate", () => {
		it("triggers scale-up when queue depth exceeds threshold", () => {
			...
			assert.ok(
				decisions.some(
					(d) => d.trigger === "queue-depth" && d.action === "scale-up",
				),
			);
		});
		...
		it("triggers warn when utilization exceeds threshold", () => {
			...
			assert.ok(
				decisions.some(
					(d) => d.trigger === "utilization" && d.action === "warn",
				),
			);
		});
		...
		it("respects cooldown period", () => {
			...
			const first = cooldownScaler.evaluate({
				queueDepth: 15,
				avgUtilization: 0,
				avgWaitMs: 0,
			});
			assert.equal(first.length, 1);
			const second = cooldownScaler.evaluate({
				queueDepth: 15,
				avgUtilization: 0,
				avgWaitMs: 0,
			});
