...
describe("ContextAssembler", () => {
  ...
  describe("addBlock", () => {
    ...
    it("adds multiple blocks across layers", () => {
      assembler.addBlock({ layer: ContextLayer.Fleet, source: "a", content: "aa", priority: 90, required: true });
      assembler.addBlock({ layer: ContextLayer.Project, source: "b", content: "bb", priority: 50, required: false });
      assembler.addBlock({ layer: ContextLayer.Task, source: "c", content: "cc", priority: 30, required: false });
  ...
  describe("assemble", () => {
    it("orders Fleet first, Project second, Task third", () => {
      assembler.addBlock({ layer: ContextLayer.Task, source: "task", content: "task content", priority: 80, required: false });
      assembler.addBlock({ layer: ContextLayer.Fleet, source: "fleet", content: "fleet content", priority: 100, required: true });
      assembler.addBlock({ layer: ContextLayer.Project, source: "project", content: "project content", priority: 60, required: false });
    ...
    it("sorts blocks within a layer by priority descending", () => {
      assembler.addBlock({ layer: ContextLayer.Fleet, source: "low", content: "low pri", priority: 10, required: false });
      assembler.addBlock({ layer: ContextLayer.Fleet, source: "high", content: "high pri", priority: 90, required: true });
    ...
    it("calculates total tokens correctly", () => {
      assembler.addBlock({ layer: ContextLayer.Fleet, source: "a", content: "x".repeat(400), priority: 50, required: true });
      assembler.addBlock({ layer: ContextLayer.Task, source: "b", content: "y".repeat(800), priority: 50, required: false });
    ...
    it("computes budgetUsed as percentage", () => {
      ...
      a.addBlock({ layer: ContextLayer.Fleet, source: "a", content: "x".repeat(2000), priority: 50, required: true });
    ...
    it("warns when budget exceeds 75%", () => {
      ...
      a.addBlock({ layer: ContextLayer.Fleet, source: "a", content: "x".repeat(320), priority: 50, required: true });
      const result = a.assemble();
    ...
    it("warns critically when budget exceeds 90%", () => {
      const a = new ContextAssembler(100);
      a.addBlock({ layer: ContextLayer.Fleet, source: "a", content: "x".repeat(380), priority: 50, required: true });
      const result = a.assemble();
      assert.ok(result.warnings.some((w) => w.includes("critically")));
    });

    it("returns no warnings when under 75%", () => {
      assembler.addBlock({ layer: ContextLayer.Fleet, source: "a", content: "short", priority: 50, required: true });
      const result = assembler.assemble();
  ...
  describe("trimToFit", () => {
    it("removes lowest-priority non-required blocks to fit budget", () => {
      ...
      a.addBlock({ layer: ContextLayer.Fleet, source: "identity", content: "x".repeat(80), priority: 100, required: true }); // 20 tokens
      a.addBlock({ layer: ContextLayer.Task, source: "low", content: "y".repeat(80), priority: 10, required: false }); // 20 tokens
      a.addBlock({ layer: ContextLayer.Task, source: "high", content: "z".repeat(80), priority: 90, required: false }); // 20 tokens
    ...
    it("never removes required blocks", () => {
      ...
      a.addBlock({ layer: ContextLayer.Fleet, source: "identity", content: "x".repeat(60), priority: 100, required: true }); // 15 tokens
      a.addBlock({ layer: ContextLayer.Task, source: "task", content: "y".repeat(20), priority: 10, required: false }); // 5 tokens
      const result = a.trimToFit();
    ...
    it("removes multiple blocks if needed", () => {
      ...
      a.addBlock({ layer: ContextLayer.Fleet, source: "req", content: "x".repeat(40), priority: 100, required: true }); // 10 tokens
      a.addBlock({ layer: ContextLayer.Task, source: "t1", content: "y".repeat(40), priority: 20, required: false }); // 10
      a.addBlock({ layer: ContextLayer.Task, source: "t2", content: "z".repeat(40), priority: 30, required: false }); // 10
      a.addBlock({ layer: ContextLayer.Task, source: "t3", content: "w".repeat(40), priority: 40, required: false }); // 10
