...
function deepMerge(base: Record<string, any>, overrides: Record<string, any>): any {
  const result = { ...base };
 ...
 */
export function loadConfig(overrides?: Partial<McpServerFullConfig>): McpServerFullConfig {
  if (!overrides) return { ...DEFAULTS, tools: { ...DEFAULTS.tools }, auth: { ...DEFAULTS.auth }, brain: { ...DEFAULTS.brain }, fleet: { ...DEFAULTS.fleet }, governance: { ...DEFAULTS.governance } };
...
export function validateConfig(config: McpServerFullConfig): string[] {
  ...
  if (typeof config.auth.tokenTtlMs !== "number" || config.auth.tokenTtlMs <= 0) {
  ...
  if (typeof config.fleet.registryPath !== "string" || !config.fleet.registryPath) {
    errors.push("fleet.registryPath must be a non-empty string");
  }

  if (typeof config.governance.standingOrdersDir !== "string" || !config.governance.standingOrdersDir) {
