...
function sampleTool(name = "test.echo", category: ToolDefinition["category"] = "system"): ToolDefinition {
  return {
    name,
...
describe("ToolRegistry", () => {
  ...
  it("throws on duplicate registration", () => {
    ...
    assert.throws(() => reg.register(sampleTool("dup"), async () => null), /already registered/);
  });
...
describe("parseRequest", () => {
  ...
  it("parses a request with params", () => {
    const raw = JSON.stringify({ jsonrpc: "2.0", id: "abc", method: "tools/call", params: { name: "foo" } });
    const req = parseRequest(raw);
