		...
		try {
			...
			if (request.method === "tools/call") {
				...
				if (!toolName) {
					this.sendJson(
						res,
						formatError(id, -32602, "Missing required param: name"),
					);
					return;
				}
			...
			this.sendJson(
				res,
				formatError(id, -32601, `Method not found: ${request.method}`),
			);
		} catch (err: unknown) {
			if (
				typeof err === "object" &&
				...
				this.sendJson(
					res,
					formatError(id, rpcErr.code, rpcErr.message, rpcErr.data),
				);
			} else {
				this.sendJson(
					res,
