	...
	registry.register(
		...
		async (params: Record<string, unknown>): Promise<unknown> => {
			...
			if (!action || !agent) {
				throw {
					code: INVALID_PARAMS,
					message: "action and agent are required",
				};
			}
			...
			for (const order of orders) {
				for (const rule of order.rules ?? []) {
					...
					const matched = conditionWords.some(
						(w) => w.length > 3 && actionLower.includes(w),
					);
	...
	registry.register(
		...
		async (params: Record<string, unknown>): Promise<unknown> => {
			...
			if (!severity || !subject || !description || !agent) {
				throw {
					code: INVALID_PARAMS,
					message: "severity, subject, description, and agent are all required",
				};
			}
