...
import { registerGovernanceTools, type StandingOrder } from "./governance-tools.js";
...
describe("Governance tools", () => {
  ...
  describe("standing_order_status", () => {
    ...
    it("includes violation counts", async () => {
      ...
      const so1 = result.orders.find((o: any) => o.name === "No Direct Production Access");
      assert.ok(so1);
  ...
  describe("escalation_file", () => {
    ...
    it("requires all params", async () => {
      await assert.rejects(
        () => registry.invoke("escalation_file", { severity: "low" }, makeContext()),
        (err: any) => err.code !== undefined,
      );
    });
  });
