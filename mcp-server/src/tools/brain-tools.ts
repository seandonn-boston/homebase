...
export function registerBrainTools(
	...
	registry.register(
		...
		async (params: Record<string, unknown>): Promise<unknown> => {
			...
			for (const file of files) {
				...
				if (
					typeof filters.since === "number" &&
					entry.createdAt < (filters.since as number)
				)
					continue;
				if (
					typeof filters.until === "number" &&
					entry.createdAt > (filters.until as number)
				)
					continue;
		...
		async (
			...
			for (const file of files) {
				...
				if (
					...
					throw {
						code: INVALID_PARAMS,
						message: `Duplicate entry: "${existing.title}" already exists (id: ${existing.id})`,
					};
				}
			}
	...
	registry.register(
		...
		async (params: Record<string, unknown>): Promise<unknown> => {
			...
			if (!id || !agent)
				throw { code: INVALID_PARAMS, message: "id and agent are required" };
	...
	registry.register(
		...
		async (
			...
			if (!id || !reason)
				throw { code: INVALID_PARAMS, message: "id and reason are required" };
