...
export function registerFleetTools(registry: ToolRegistry, registryPath: string): void {
  // -----------------------------------------------------------------------
  // fleet_status — agent+
  // -----------------------------------------------------------------------
  registry.register(
  ...
  registry.register(
    ...
    async (params: Record<string, unknown>): Promise<unknown> => {
      ...
      for (const agent of agents) {
        ...
        if (reqCaps.length > 0) {
          const overlap = reqCaps.filter((c) => agent.capabilities.includes(c)).length;
    ...
    async (params: Record<string, unknown>): Promise<unknown> => {
      ...
      const scored = agents.map((agent) => {
        ...
        if (requiredCaps.length > 0) {
          const overlap = requiredCaps.filter((c) => agent.capabilities.includes(c)).length;
          score += (overlap / requiredCaps.length) * 50;
          if (overlap > 0) reasons.push(`${overlap}/${requiredCaps.length} capabilities match`);
        } else {
