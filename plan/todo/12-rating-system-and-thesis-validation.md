...
- [x] **TV-02: Advisory vs enforcement comparison** — Implement A/B framework running identical tasks under advisory-only and hard-blocking modes. Measure violation rate, violation severity, first-pass quality, completion time, and escalation rate. Minimum 10 task pairs with statistical significance methodology. *(Implemented: `admiral/thesis/ab-framework.ts` — A/B framework with 5 metrics, statistical significance assessment, verdict determination, task pair management, 16 tests, 908ms)*
- [x] **TV-03: Before/after case studies** — Document real instances where enforcement caught problems advisory guidance would have missed. Each case study includes counterfactual analysis and true positive / false positive classification. At least 5 case studies from actual Helm development. *(Implemented: `admiral/thesis/case-studies.ts` — case study registry, TP/FP classification, impact assessment, index with aggregates, markdown formatter, 3 tests)*
- [x] **TV-04: Agent quality improvement tracking** — Track agent output quality longitudinally correlated with governance maturity. Correlate first-pass quality, defect density, and escalation rate with specific governance additions. At least 3 governance milestones with before/after data. *(Implemented: `admiral/thesis/quality-tracking.ts` — snapshot/milestone tracking, before/after correlation, trend computation, 3 tests)*

## Thesis Evidence & Cost

- [x] **TV-05: Developer experience survey framework** — Define short (< 5 minutes) survey covering friction, trust, productivity impact, and perceived value. Adapt Net Promoter Score for governance. Quarterly administration cadence with anonymization. *(Implemented: `admiral/thesis/survey-framework.ts` — 9 questions across 4 categories, NPS scoring, category averages, 3 tests)*
- [x] **TV-06: False positive tracking** — Track every instance where hooks block legitimate actions. Capture per-incident details and aggregate per-hook false positive rates, developer-hours lost, and mean time to resolve. High false positive rates undermine the thesis. *(Implemented: `admiral/thesis/false-positive-tracker.ts` — per-incident logging, per-hook rates, blocked hours, tuning tracking, 2 tests)*
- [x] **TV-07: Governance overhead measurement** — Measure hook execution latency (p50/p95/p99), governance token ratio, governance-induced delays, context window overhead, and developer interruption cost. Compare against the 25% governance overhead hard cap. *(Implemented: `admiral/thesis/overhead-measurement.ts` — 5 overhead categories, percentile stats, hard cap check, 4 tests)*
- [x] **TV-08: ROI calculation model** — Quantify costs (implementation, maintenance, runtime, friction) and benefits (bugs prevented, incidents avoided, rework reduction, knowledge preservation, onboarding acceleration). Produce breakeven analysis with sensitivity analysis on key assumptions. *(Implemented: `admiral/thesis/roi-model.ts` — 4 cost + 5 benefit categories, breakeven, sensitivity analysis, 5 tests)*
- [x] **TV-09: Academic paper outline** — Outline for research paper covering thesis, methodology, and evidence across 8 sections. Conference paper length (8000-12000 words). Detailed enough for independent section drafting.
---
