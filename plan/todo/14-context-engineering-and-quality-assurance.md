...
- [x] **QA-02: Test generation framework** — Generate test skeletons for new `.ts` and `.sh` files with describe/it blocks for each public function, edge case placeholders, and project convention adherence; skeletons must compile/parse without errors; callable standalone and integratable into CI. *(Implemented: `admiral/quality/test-generator.ts` — TS and shell parsers, skeleton generation with edge case placeholders, 30 tests, 203ms)*
...
- [x] **QA-04: Quality metrics collection** — Collect per-module metrics (cyclomatic complexity, test coverage, code churn, defect density, lint violations, test-to-code ratio) with timestamps; output JSON consumable by the control plane dashboard; accumulate historical data across CI runs. *(Implemented: `admiral/quality/quality-metrics.ts` — 6 metric dimensions, history accumulation, text/JSON formatters, 21 tests, 2.1s)*
- [x] **QA-08: Review checklist automation** — Generate risk-appropriate review checklists based on changed files; classify file risk (hooks/config = high, tests/docs = low); include domain-specific items; pre-fill automatically verifiable items; output as markdown for PR descriptions. *(Implemented: `admiral/quality/review-checklist.ts` — 14 category templates, risk classification, markdown output, 35 tests, 1.1s)*
...
- [x] **QA-06: Technical debt tracker** — Scan five debt sources (TODO/FIXME/HACK comments, high-complexity modules, skipped tests, vulnerable/outdated dependencies, code duplication); estimate remediation effort (S/M/L); produce prioritized backlog sorted by risk and a debt ratio metric. *(Implemented: `admiral/quality/debt-tracker.ts` — 5 debt sources, prioritized backlog, debt ratio, effort estimates, 23 tests, 0.8s)*
- [x] **QA-07: Code complexity budget** — Enforce per-module maximum cyclomatic complexity; block over-budget commits in CI; implement a complexity credit system (net complexity must not increase); generate refactoring recommendations; budgets ratchet — loosening requires Admiral approval. *(Implemented: `admiral/quality/complexity-budget.ts` — per-module budgets, credit system, ratchet, refactoring recommendations, 22 tests, 0.9s)*
- [x] **QA-09: Quality score per module** — Aggregate six dimensions into a 0-100 composite score: test coverage (25%), complexity compliance (20%), lint cleanliness (15%), documentation completeness (15%), defect density (15%), code churn stability (10%); classify green/yellow/red; track over time with trend indicators. *(Implemented: `admiral/quality/quality-score.ts` — 6-dimension composite scoring, green/yellow/red grades, weighted aggregation, 24 tests, 0.7s)*
- [x] **QA-10: Quality regression prevention** — CI gate that blocks merges dropping a module below the red threshold (60) or declining by more than 10 points; exception via quality-debt acknowledgment label requiring Admiral approval; post quality impact report to the PR; run in < 60s. *(Implemented: `admiral/quality/regression-prevention.ts` — red threshold gate, decline detection, debt acknowledgment, PR quality impact markdown, baseline extraction, 21 tests, 0.9s)*
