...
- [ ] **T-20: Snapshot tests for hook JSON output schemas** — *Deferred to Phase 2 (depends on A-01 schema validation).* — Capture JSON output schema of every hook adapter as `.snap.json` files. Done when CI fails on snapshot mismatch.
