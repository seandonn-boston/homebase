...
- [x] **T-16: Mutation testing setup** — *Completed in Phase 9.* — Installed `@stryker-mutator/core` and `@stryker-mutator/typescript-checker`. Created `stryker.config.json` targeting `ring-buffer.ts` and `events.ts` as pilot modules, with command runner (`node --test`), 80% break threshold, JSON+HTML+clear-text reporters. Added `test:mutation` npm script.
...
- [x] **Q-03: Document and enforce hook header standard** — *Completed in Phase 9.* — Header standard defined in ADMIRAL_STYLE.md (D-01). CI validation script `test_hook_headers.sh` checks: shebang, "Admiral Framework" header, comment density, `set -euo pipefail`, exit behavior documentation. Covers both `.hooks/` and `admiral/lib/`.
...
- [x] **Q-10: Consistent logging format across all bash scripts** — *Completed in Phase 9.* — Structured JSON logging via `admiral/lib/log.sh` (`log_structured`/`log_json`/`log_info`/`log_warn`/`log_error`). Fields: timestamp, level, component, correlation_id, message, context. All hooks that use `hook_utils.sh` (12 hooks) produce structured logs via `hook_log()` which delegates to `log_structured()`. Added `log_json()` alias.
...
- [x] **Q-14: Hook idempotency verification** — *Completed in Phase 9.* — Test suite runs each hook twice with identical input and verifies: exit codes match, normalized outputs match (timestamps stripped). Covers 13 hooks: loop_detector, zero_trust_validator, compliance_ethics_advisor, token_budget_tracker, context_health_check, scope_boundary_guard, prohibitions_enforcer, tool_permission_guard, post/pre adapters.
...
- [x] **EDD-03: Wire EDD gate into `/next-todo`** — *Completed in Phase 9.* — Added step 2 to Execution Algorithm: if the most recently completed task has an eval spec in `.admiral/edd-specs/`, run `edd_gate <task-id> --confirm-probabilistic`. Gate failure reports missing checks instead of starting new work. Specs are optional.
- [x] **EDD-04: Proof-of-completion artifact** — *Completed in Phase 9.* — Added `--save-proof` flag to `edd_gate`. Writes `{task_id}.proof.json` to `.admiral/proofs/` with task ID, timestamp, deterministic results (command + exit code + output hash), probabilistic confirmations, CI status, and CI run URL.
- [x] **EDD-05: Visual confirmation protocol** — *Completed in Phase 9.* — Created `admiral/lib/edd_confirm.sh` with `edd_confirm()` (record confirmation with reviewer, verdict, evidence path, auto-detect human/agent reviewer type), `edd_is_confirmed()`, `edd_get_confirmations()`, `edd_capture_evidence()`. 14 test cases. Stored in `.admiral/confirmations/`.
