...
describe("ClaudeCodeAdapter", () => {
  ...
  it("injectContext throws when not initialized", async () => {
    await assert.rejects(
      () => adapter.injectContext("helm", makeContext()),
...
describe("HeadlessAdapter", () => {
  ...
  it("injectContext uses programmatic format", async () => {
    ...
    assert.ok(adapter._lastInjectedContext.includes("[standing] identity: helm"));
    assert.ok(adapter._lastInjectedContext.includes("[session] project: technomancy"));
    assert.ok(adapter._lastInjectedContext.includes("[working] task: implement PA-01"));
  });
