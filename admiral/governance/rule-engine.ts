		...
		for (const rule of this.rules.values()) {
			...
			if (evaluation.triggered) {
				this.bus.emit({
					...
					severity:
						rule.action.level >= InterventionLevel.Suspend
							? "critical"
							: "high",
	...
	private evaluateComposite(
		...
		for (const sub of cond.conditions) {
			...
			if ("metric" in sub && "operator" in sub) {
				const eval_ = this.evaluateThreshold(
					{ ...rule, conditionType: "threshold", condition: sub },
					metrics,
				);
				subTriggered = eval_.triggered;
				evidence = eval_.evidence;
			} else if ("eventTypes" in sub) {
				const eval_ = this.evaluatePattern(
					{ ...rule, conditionType: "pattern", condition: sub },
					events,
				);
				subTriggered = eval_.triggered;
				evidence = eval_.evidence;
			} else if ("metric" in sub && "changePercent" in sub) {
				const eval_ = this.evaluateTemporal(
					{ ...rule, conditionType: "temporal", condition: sub },
					metrics,
				);
				subTriggered = eval_.triggered;
				evidence = eval_.evidence;
