...
export class ArbiterAgent extends GovernanceAgent {
	...
	detectConflicts(events: GovernanceEvent[]): Conflict[] {
		...
		for (const [taskId, group] of taskGroups) {
			const agents = [
				...new Set(group.map((e) => e.sourceAgent).filter(Boolean)),
			] as string[];
				...
				conflicts.push({
					...
					evidence: {
						...
						agentOutputs: Object.fromEntries(
							group.map((e) => [e.sourceAgent, e.data.output]),
						),
					},
					timestamp: Date.now(),
				});
		...
		for (const event of events) {
			if (
				...
				continue;
			const requestors =
				interventionTargets.get(event.targetAgent) ?? new Set();
	...
	resolveByEvidence(
		...
		const resolution: Resolution = {
			...
			winner,
			rationale:
				(evidence.rationale as string) ?? "Resolved by evidence analysis",
			resolvedBy: this.agentId,
			timestamp: Date.now(),
		};
		this.resolutions.push(resolution);
