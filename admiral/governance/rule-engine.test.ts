...
import { GovernanceRule, GovernanceRuleEngine } from "./rule-engine";
  ...
  describe("pattern rules", () => {
    it("triggers when event count meets threshold in window", () => {
      const rule: GovernanceRule = {
        ...
        condition: { eventTypes: ["agent_failure"], minOccurrences: 3, windowMs: 60_000 },
        action: { level: InterventionLevel.Restrict, message: "Too many failures" },
        version: 1,
        lastModified: now,
      };
      engine.addRule(rule);
    ...
    it("does not trigger when below occurrence count", () => {
      const rule: GovernanceRule = {
        ...
        enabled: true,
        conditionType: "pattern",
        condition: { eventTypes: ["agent_failure"], minOccurrences: 5, windowMs: 60_000 },
  ...
  describe("temporal rules", () => {
    it("triggers on metric increase above threshold", () => {
      const rule: GovernanceRule = {
        ...
        condition: { metric: "cost", changePercent: 50, windowMs: 300_000, direction: "increase" },
    ...
    it("detects decrease direction", () => {
      const rule: GovernanceRule = {
        ...
        enabled: true,
        conditionType: "temporal",
        condition: { metric: "health", changePercent: 30, windowMs: 60_000, direction: "decrease" },
        action: { level: InterventionLevel.Restrict, message: "Health dropping" },
        version: 1,
        lastModified: now,
      };
      engine.addRule(rule);
  ...
  describe("composite rules", () => {
    it("AND: triggers only when all sub-conditions met", () => {
      const rule: GovernanceRule = {
        ...
        action: { level: InterventionLevel.Suspend, message: "Resource overload" },
        version: 1,
        lastModified: now,
      };
      engine.addRule(rule);
