...
export class ComplianceMonitorAgent extends GovernanceAgent {
  ...
  analyze(events: GovernanceEvent[]): GovernanceFinding[] {
    ...
    for (const compliance of allCompliance) {
      const score = Math.max(compliance.mechanicalCompliance, compliance.judgmentCompliance);
      if (score < 50) {
        findings.push({
          ...
          evidence: { orderId: compliance.orderId, score, violationCount: compliance.violationCount },
  ...
  checkMechanicalCompliance(events: GovernanceEvent[]): StandingOrderCompliance[] {
  ...
  checkJudgmentCompliance(events: GovernanceEvent[]): StandingOrderCompliance[] {
    const results: StandingOrderCompliance[] = [];
    ...
    for (const order of this.standingOrders) {
      ...
      const orderEvents = events.filter(
        (e) => e.data.orderId === order.id,
      );

      const compliant = orderEvents.filter((e) => e.data.compliant === true).length;
      const nonCompliant = orderEvents.filter((e) => e.data.compliant === false).length;
      const total = compliant + nonCompliant;
      const score = total > 0 ? Math.round((compliant / total) * 100) : 100;

      const violationEvents = orderEvents.filter((e) => e.data.compliant === false);
      const lastViolation = violationEvents.length > 0
  ...
  generateReport(windowMs: number = 3_600_000): ComplianceReport {
    ...
    const scores = allOrders.map((o) => Math.max(o.mechanicalCompliance, o.judgmentCompliance));
