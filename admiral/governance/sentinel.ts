...
export class SentinelAgent extends GovernanceAgent {
    ...
    for (const [key, group] of groups) {
      if (group.length >= this.config.loopThreshold) {
        ...
        findings.push({
          ...
          evidence: { count: group.length, window: this.config.loopWindow, sampleEventId: group[0].id },
  ...
  detectBudgetAnomaly(events: GovernanceEvent[]): GovernanceFinding[] {
    ...
    if (recentRate > avgRate * this.config.budgetMultiplier && recent.length >= 2) {
      const agentIds = [...new Set(recent.map((e) => e.sourceAgent ?? "unknown"))];
      for (const agentId of agentIds) {
        findings.push({
          ...
          evidence: { recentRate, avgRate, multiplier: recentRate / avgRate, recentCount: recent.length },
          recommendedAction: InterventionLevel.Restrict,
        });
      }
    }
    ...
    for (const [agent, group] of groups) {
      if (group.length >= this.config.scopeThreshold) {
        findings.push({
          ...
          evidence: { count: group.length, threshold: this.config.scopeThreshold },
          recommendedAction: InterventionLevel.Restrict,
        });
      }
    }
  ...
  startMonitoring(intervalMs: number = 10_000): void {
    ...
    this.monitoringTimer = setInterval(() => {
      const events = this.bus.getRecentEvents(Math.max(
