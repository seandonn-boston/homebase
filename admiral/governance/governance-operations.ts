...
import type { GovernanceEventBus, GovernanceEvent } from "./framework";
import type { InterventionProtocol, Intervention } from "./intervention";
...
export class GovernanceAuditDashboard {
  ...
  constructor(bus: GovernanceEventBus, interventionProtocol: InterventionProtocol) {
    this.bus = bus;
    this.interventionProtocol = interventionProtocol;
  }

  getActiveFindingsView(): { type: string; severity: string; agent: string; timestamp: number }[] {
    const events = this.bus.getHistory({
      types: ["scope_drift", "budget_overrun", "loop_detected", "trust_violation", "compliance_finding"],
    });
    return events.map((e) => ({
      type: e.type,
      severity: e.severity,
  ...
  getGovernanceAgentHealth(): { agentId: string; healthy: boolean; lastActivity: number }[] {
    const events = this.bus.getHistory();
...
export class GovernanceSelfGovernance {
  ...
  validateNoSelfModification(
    ...
    if (isSelfTarget) {
      return { allowed: false, reason: "Self-modification of governance configuration is prohibited" };
    }
...
export class GovernanceMetrics {
  private bus: GovernanceEventBus;
  private interventionProtocol: InterventionProtocol;
  private detectionTimestamps: Map<string, { eventTime: number; findingTime: number }> = new Map();
  private resolutionTimestamps: Map<string, { findingTime: number; resolvedTime: number }> = new Map();
  ...
  getComplianceScore(): number {
    ...
    const findings = events.filter((e) => e.type === "compliance_finding").length;
  ...
  recordResolution(id: string, findingTime: number, resolvedTime: number): void {
...
export class MultiOperatorGovernance {
  ...
  addOperator(operatorId: string, level: "owner" | "operator" | "observer"): OperatorRole {
    if (this.operators.size >= this.maxOperators && !this.operators.has(operatorId)) {
  ...
  resolveConflict(
    ...
    if (rankA > rankB) {
      return { winner: operatorA, reason: `Higher authority: ${roleA.level} > ${roleB.level}` };
    }
    if (rankB > rankA) {
      return { winner: operatorB, reason: `Higher authority: ${roleB.level} > ${roleA.level}` };
    }
    // Same tier: conservative action wins (operatorB is assumed to be the blocker)
    return { winner: operatorB, reason: "Same authority level — conservative action prevails" };
  }
...
export class FallbackDecomposer {
  ...
  decompose(task: string): { subtasks: string[]; routing: Record<string, string> } {
    if (!this.active) {
