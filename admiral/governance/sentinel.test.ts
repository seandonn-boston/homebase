...
describe("SentinelAgent", () => {
  ...
  describe("detectLoops", () => {
    ...
    it("groups by agent and data pattern separately", () => {
      const events: GovernanceEvent[] = [
        { id: "e1", timestamp: now - 5000, type: "scope_drift", sourceAgent: "agent-a", severity: "low", data: { file: "a.ts" } },
        { id: "e2", timestamp: now - 4000, type: "scope_drift", sourceAgent: "agent-a", severity: "low", data: { file: "b.ts" } },
        { id: "e3", timestamp: now - 3000, type: "scope_drift", sourceAgent: "agent-a", severity: "low", data: { file: "a.ts" } },
        { id: "e4", timestamp: now - 2000, type: "scope_drift", sourceAgent: "agent-a", severity: "low", data: { file: "a.ts" } },
      ];
  ...
  describe("detectBudgetAnomaly", () => {
    it("detects high burn rate in recent window", () => {
      ...
      const historical: GovernanceEvent[] = Array.from({ length: 5 }, (_, i) => ({
        id: `h${i}`,
    ...
    it("returns empty for no budget events", () => {
      const events: GovernanceEvent[] = [{
        id: "e1", timestamp: now, type: "scope_drift", severity: "low", data: {},
      }];
      const findings = sentinel.detectBudgetAnomaly(events);
      assert.equal(findings.length, 0);
    });
