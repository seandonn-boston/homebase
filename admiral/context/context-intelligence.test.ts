...
describe("ContextOverflowHandler (CE-07)", () => {
  ...
  it("emergency at 95% keeps only standing + task", () => {
    manager.addItem(ContextZone.Standing, "identity".repeat(100), { priority: 100 });
    manager.addItem(ContextZone.Session, "x".repeat(240_000), { priority: 50 });
    manager.addItem(ContextZone.Working, "task data".repeat(100), { priority: 90 });
    manager.addItem(ContextZone.Working, "extra work".repeat(50), { priority: 20 });

    const { freedTokens, removedItems, remainingItems } = handler.handleEmergency();
    assert.ok(freedTokens > 0);
    assert.ok(removedItems > 0);
    // Should keep standing items + highest-priority working item
    const remaining = manager.getItems();
    const standingRemaining = remaining.filter((i) => i.zone === ContextZone.Standing);
...
describe("DynamicContextAllocator (CE-08)", () => {
  ...
  it("standing hard limit is respected in all profiles", () => {
    for (const complexity of ["simple", "standard", "complex"] as const) {
      ...
      const profile = allocator.getCurrentProfile();
      assert.ok(profile.standing.max <= 25, `${complexity} standing max should be <= 25`);
    }
  });
...
describe("ContextPreloader (CE-09)", () => {
  ...
  it("preload within budget succeeds", () => {
    const sources: PreloadSource[] = [
      { type: "file-dependency", content: "dep content", priority: 80, tokenEstimate: 10 },
      { type: "historical", content: "history data", priority: 60, tokenEstimate: 10 },
    ];
  ...
  it("preload over budget skips items", () => {
    ...
    const sources: PreloadSource[] = [
      { type: "file-dependency", content: "a".repeat(80), priority: 80, tokenEstimate: 20 }, // 20 tokens
      { type: "historical", content: "b".repeat(80), priority: 60, tokenEstimate: 20 }, // 20 tokens
    ];
...
describe("ContextAuditTrail (CE-10)", () => {
  ...
  it("captures a snapshot with correct structure", () => {
    manager.addItem(ContextZone.Session, "test data", { priority: 50, source: "test" });
  ...
  it("diffs between two snapshots", () => {
    manager.addItem(ContextZone.Session, "item A", { priority: 50, source: "test" });
    const snapA = trail.capture("checkpoint", "agent-1");

    // Add another item
    manager.addItem(ContextZone.Session, "item B", { priority: 60, source: "test" });
