...
describe("ContextProfileManager", () => {
  ...
  describe("sacrifice order", () => {
    ...
    it("orders oldest first on tie", () => {
      const item1 = manager.addItem(ContextZone.Working, "first", { priority: 50 });
      const item2 = manager.addItem(ContextZone.Working, "second", { priority: 50 });
  ...
  describe("evictToFit", () => {
    it("evicts items until under target", () => {
      ...
      for (let i = 0; i < 10; i++) {
        manager.addItem(ContextZone.Working, "x".repeat(400), { priority: i * 10 });
      }
  ...
  describe("getItems", () => {
    ...
    it("returns only items for specified zone", () => {
      ...
      assert.equal(manager.getItems(ContextZone.Session)[0].zone, ContextZone.Session);
    });
  });
