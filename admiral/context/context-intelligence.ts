...
export class ContextDashboard {
	...
	takeSnapshot(): UtilizationDashboardData {
		...
		const byZone: Record<
			string,
			{ used: number; max: number; percent: number }
		> = {};
		for (const zone of [
			ContextZone.Standing,
			ContextZone.Session,
			ContextZone.Working,
		]) {
	...
	detectAnomalies(): string[] {
		...
		if (this.snapshots.length >= 2) {
			...
			if (prevWorking && currWorking.max > 0) {
				const growth =
					((currWorking.used - prevWorking.used) / currWorking.max) * 100;
				if (growth > 5) {
...
export class ContextOverflowHandler {
		...
		this.actions = [
			...
			},
			{
				...
				actions: [
					...
					"keep standing + current task only",
				],
				triggered: false,
			},
			{
	...
	handleHandoff(): { checkpointData: string; reason: string } {
		...
		const reason =
			"Context utilization at 100% — forced checkpoint and session handoff";
...
export class DynamicContextAllocator {
	...
	getCurrentProfile(): AllocationProfile {
		const profile = this.profiles.find(
			(p) => p.complexity === this.currentComplexity,
		);
...
export class ContextPreloader {
	...
	preload(taskType: string): {
		loaded: PreloadSource[];
		skipped: PreloadSource[];
		reason?: string;
	} {
		...
		if (!sources || sources.length === 0) {
			return {
				loaded: [],
				skipped: [],
				reason: "no predictions for task type",
			};
		}
		...
		const reason =
			skipped.length > 0
				? "some items skipped due to budget constraints"
				: undefined;
...
export class ContextAuditTrail {
	...
	getCompressionHistory(
		since?: number,
	): { itemId: string; action: string; timestamp: number }[] {
		if (since != null) {
