...
export class ContextProfileManager {
	...
	private buildZones(budget: number): Record<ContextZone, ZoneAllocation> {
		...
		for (const zone of [
			...
			result[zone] = {
				...
				maxTokens:
					cfg.hardLimit != null
						? Math.min(computedMax, cfg.hardLimit)
						: computedMax,
	...
	addItem(
		...
		const compressible =
			zone === ContextZone.Standing ? false : (options.compressible ?? true);
	...
	> {
		...
		for (const zone of [
			...
			result[zone] = {
				...
				percent:
					alloc.maxTokens > 0
						? (alloc.currentTokens / alloc.maxTokens) * 100
						: 0,
			};
		}
		return result;
