...
describe("ContextBudgetTracker (CE-02)", () => {
	...
	describe("checkThresholds", () => {
		...
		it("returns warning at 75%+ usage", () => {
			...
			manager.addItem(ContextZone.Session, "x".repeat(240_000), {
				priority: 50,
			}); // ~60K tokens
			manager.addItem(ContextZone.Working, "y".repeat(64_000), {
				priority: 30,
			}); // ~16K tokens
			const result = tracker.checkThresholds();
		...
		it("returns emergency at 90%+ usage", () => {
			...
			manager.addItem(ContextZone.Working, "y".repeat(112_000), {
				priority: 30,
			}); // 28K tokens
			manager.addItem(ContextZone.Standing, "z".repeat(16_000), {
				priority: 90,
			}); // 4K tokens
			const result = tracker.checkThresholds();
...
describe("ContextCompressor (CE-03)", () => {
	...
	describe("compress with Summarize strategy", () => {
		it("truncates compressible items", () => {
			manager.addItem(ContextZone.Session, "a".repeat(800), {
				priority: 20,
				compressible: true,
			});
...
describe("RelevanceScorer (CE-04)", () => {
	...
	describe("score", () => {
		...
		it("gives higher authority weight to Standing zone items", () => {
			const standing = scorer.score(
				makeItem({ zone: ContextZone.Standing, priority: 50 }),
			);
			const working = scorer.score(
				makeItem({ zone: ContextZone.Working, priority: 50 }),
			);
			assert.ok(
				standing.dimensions.authorityWeight >
					working.dimensions.authorityWeight,
			);
		});

		it("gives higher dependency score to non-compressible items", () => {
			const required = scorer.score(
				makeItem({ compressible: false, priority: 50 }),
			);
			const optional = scorer.score(
				makeItem({ compressible: true, priority: 50 }),
			);
			assert.ok(
				required.dimensions.dependency >= optional.dimensions.dependency,
			);
		});
		...
		it("scores semantic proximity higher with matching task", () => {
			const item = makeItem({
				...
				content: "routing engine handles task distribution",
			});
			const withTask = scorer.score(item, "routing engine distribution");
			const noTask = scorer.score(item);
			assert.ok(
				withTask.dimensions.semanticProximity >=
					noTask.dimensions.semanticProximity,
			);
		});
	...
	describe("rankItems", () => {
		it("returns items sorted by score descending", () => {
			const items = [
				makeItem({
					zone: ContextZone.Working,
					priority: 10,
					compressible: true,
				}),
				makeItem({
					zone: ContextZone.Standing,
					priority: 90,
					compressible: false,
				}),
				makeItem({
					zone: ContextZone.Session,
					priority: 50,
					compressible: true,
				}),
			];
	...
	describe("dimension ranges", () => {
		it("recency is 0-20", () => {
			...
			const result = scorer.score(item);
			assert.ok(
				result.dimensions.recency >= 0 && result.dimensions.recency <= 20,
			);
		});
		...
		it("frequency is 0-20", () => {
			...
			const result = scorer.score(item);
			assert.ok(
				result.dimensions.frequency >= 0 && result.dimensions.frequency <= 20,
			);
		});
		...
		it("authorityWeight is 0-20", () => {
			...
			const result = scorer.score(item);
			assert.ok(
				result.dimensions.authorityWeight >= 0 &&
					result.dimensions.authorityWeight <= 20,
			);
		});
		...
		it("dependency is 0-20", () => {
			...
			const result = scorer.score(item);
			assert.ok(
				result.dimensions.dependency >= 0 && result.dimensions.dependency <= 20,
			);
		});
	});
});
...
describe("ContextInjector (CE-05)", () => {
	...
	describe("assembleContext", () => {
		it("orders Standing first, Session middle, Working last", () => {
			manager.addItem(ContextZone.Working, "task data", {
				priority: 80,
				source: "task",
			});
			...
			manager.addItem(ContextZone.Session, "reference", {
				priority: 60,
				source: "reference",
			});
		...
		it("sorts within zone by relevance score", () => {
			manager.addItem(ContextZone.Session, "low pri", {
				priority: 10,
				source: "low",
			});
			manager.addItem(ContextZone.Session, "high pri", {
				priority: 90,
				source: "high",
			});
			const { ordered } = injector.assembleContext();
			const sessionItems = ordered.filter(
				(i) => i.zone === ContextZone.Session,
			);
		...
		it("preserves primacy/recency ordering even with task context", () => {
			manager.addItem(ContextZone.Working, "implement routing feature", {
				priority: 95,
				source: "task",
			});
			manager.addItem(ContextZone.Standing, "I am helm", {
				priority: 100,
				source: "identity",
			});
