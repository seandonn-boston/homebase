...
export class RelevanceScorer {
	...
	score(item: ContextItem, currentTask?: string): RelevanceScore {
		...
		return {
			itemId: item.id,
			score:
				recency + frequency + semanticProximity + authorityWeight + dependency,
			dimensions: {
				recency,
				frequency,
				semanticProximity,
	...
	private scoreSemanticProximity(
		item: ContextItem,
		currentTask?: string,
	): number {
...
export class ContextInjector {
	...
	assembleContext(currentTask?: string): {
		ordered: ContextItem[];
		totalTokens: number;
	} {
